...
const contextCommand: CommandHandler = (cmd, ctx) => {
  ...
  if (cmd === "/usage") {
    ...
    const main = ctx.tracker.totals("main");
    const sub = ctx.tracker.totals("subagent");
    console.log("\n=== 用量统计 ===");
    ...
    console.log(`  ├─ 主 Agent: ${main.totalTokens} (入 ${main.inputTokens + main.cacheReadTokens + main.cacheWriteTokens} + 出 ${main.outputTokens})`);
    console.log(`  └─ 子 Agent: ${sub.totalTokens} (入 ${sub.inputTokens + sub.cacheReadTokens + sub.cacheWriteTokens} + 出 ${sub.outputTokens})`);
    console.log(`  实际成本: $${totals.cost.toFixed(6)} (主 $${main.cost.toFixed(6)} + 子 $${sub.cost.toFixed(6)})`);
